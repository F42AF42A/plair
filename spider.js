/* Паук, который ходит за курсором по странице «О студии».
 *
 ...
 *
 * Холст растянут на весь экран, так что ходить пауку есть где.
 * Цвет наш; на тач-экранах и при «уменьшить движение» не включается.
 */
export function mount(host) {
  ...
  // Ноги: своя точка опоры, своя цель, свой подъём в шаге.
  ...
  function step(now) {
    ...
    for (let i = 0; i < LEGS; i++) {
      ...
      const rx = hx + Math.cos(a) * leg.reach;
      ...
      const kx = mx + Math.cos(a) * 20;
      const ky = my + Math.sin(a) * 20 - 26 - lift;
